...
func authorizeReadSystemBucket(ctx context.Context, orgID, id influxdb.ID, u influxdb.UserResourceMappingService) error {
	// HACK: remove once system buckets are migrated away from hard coded values
	if id == influxdb.TasksSystemBucketID || id == influxdb.MonitoringSystemBucketID {
		return nil
	}
...
func (s *BucketService) FindBuckets(ctx context.Context, filter influxdb.BucketFilter, opt ...influxdb.FindOptions) ([]*influxdb.Bucket, int, error) {
	...
	for _, b := range bs {
		...
		case influxdb.BucketTypeSystem:
			err = authorizeReadSystemBucket(ctx, b.OrgID, b.ID, s.u)
		default:
